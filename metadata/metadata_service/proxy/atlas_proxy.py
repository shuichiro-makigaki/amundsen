...
class AtlasProxy(BaseProxy):
    ...
    def _get_table_entity(self, *, table_uri: str) -> AtlasEntityWithExtInfo:
        ...
        try:
            return self.client.entity.get_entity_by_attribute(type_name=table_info['entity'],
                                                              uniq_attributes=[(CommonKeys.qn, table_qn)])
    ...
    def _get_user_entity(self, user_id: str) -> AtlasEntityWithExtInfo:
        ...
        try:
            return self.client.entity.get_entity_by_attribute(type_name=CommonTypes.user,
                                                              uniq_attributes=[(CommonKeys.qn, user_id)])
        except Exception as ex:
        ...
        bookmark_entity = {
            'entity': {
                ...
                'attributes': {'qualifiedName': bookmark_qn,
                               ...
                               'entityUri': table_uri,
                               'user': {'guid': user_guid},
                               'entity': {'guid': entity.entity[CommonKeys.guid]}}
            }
        }
    ...
    def _get_column(self, *, table_uri: str, column_name: str) -> Dict:
        ...
        try:
            ...
            columns = table_entity.entity[CommonKeys.rels].get('columns')
            for column in columns or list():
                col_details = table_entity.referredEntities[column[CommonKeys.guid]]
                if column_name == col_details[CommonKeys.attrs]['name']:
                    return col_details
    ...
    def _serialize_columns(self, *, entity: AtlasEntityWithExtInfo) -> \
        ...
        """
        columns = list()
        for column in entity.entity[CommonKeys.rels].get('columns') or list():
            column_status = column.get('entityStatus', 'inactive').lower()

            if column_status != 'active':
                continue

            col_entity = entity.referredEntities[column[CommonKeys.guid]]
            col_attrs = col_entity[CommonKeys.attrs]
            statistics = list()
    ...
    def get_table(self, *, table_uri: str) -> Table:
        ...
        try:
            attrs = table_details[CommonKeys.attrs]
            ...
            table_qn = parse_table_qualified_name(
                qualified_name=attrs.get(CommonKeys.qn)
            )
            ...
            table = Table(
                ...
                owners=self._get_owners(
                    table_details[CommonKeys.rels].get('ownedBy', []), attrs.get('owner')),
    ...
    def get_table_description(self, *,
        ...
        """
        entity = self._get_table_entity(table_uri=table_uri)
        return entity.entity[CommonKeys.attrs].get('description')
    ...
    def _get_user_defined_glossary_guid(self) -> str:
        ...
        for glossary in glossaries:
            if glossary.get(CommonKeys.qn) == self.AMUNDSEN_USER_TAGS:
                return glossary[CommonKeys.guid]
    ...
    def add_tag(self, *, id: str, tag: str, tag_type: str = "default",
        ...
        related_entity = AtlasRelatedObjectId({CommonKeys.guid: entity.entity[CommonKeys.guid],
    ...
    def delete_tag(self, *, id: str, tag: str, tag_type: str,
        ...
        for item in assigned_entities or list():
            if item.get(CommonKeys.guid) == entity.entity[CommonKeys.guid]:
    ...
    def put_column_description(self, *,
        ...
        col_guid = column_detail[CommonKeys.guid]
    ...
    def get_column_description(self, *,
        ...
        column_detail = self._get_column(
            table_uri=table_uri,
            column_name=column_name)
        return column_detail[CommonKeys.attrs].get('description')
    ...
    def _serialize_popular_tables(self, entities: AtlasEntitiesWithExtInfo) -> List[PopularTable]:
        ...
        for table in entities.entities:
            ...
            table_qn = parse_table_qualified_name(
                qualified_name=table_attrs.get(CommonKeys.qn)
            )
    ...
    def _get_resources_followed_by_user(self, user_id: str, resource_type: str) \
        ...
        params = {
            ...
            'attributes': ['count', CommonKeys.qn, CommonKeys.uri]
        }
        ...
        for record in search_results.entities:
            if resource_type == ResourceType.Table.name:
                table_info = self._extract_info_from_uri(table_uri=record.attributes[CommonKeys.uri])
                res = self._parse_bookmark_qn(record.attributes[CommonKeys.qn])
    ...
    def _get_resources_owned_by_user(self, user_id: str, resource_type: str) \
        ...
        resource_guids = set()
        for item in self._filter_active(user_entity[CommonKeys.rels].get('owns')) or list():
            if re.compile(type_regex).match(item['typeName']):
                resource_guids.add(item[CommonKeys.guid])
    ...
    def get_frequently_used_tables(self, *, user_email: str) -> Dict[str, List[PopularTable]]:
        user = self.client.entity.get_entity_by_attribute(type_name=CommonTypes.user,
                                                          uniq_attributes=[(CommonKeys.qn, user_email)]).entity
        ...
        for reader in readers.entities or list():
            entity_uri = reader.attributes.get(CommonKeys.uri)
    ...
    def _get_dashboard_summary(self, entity: AtlasEntity, executions: List[AtlasEntity]) -> Dict:
        attributes = entity[CommonKeys.attrs]
        relationships = entity[CommonKeys.rels]

        group = self._get_dashboard_group(relationships.get('group').get('guid'))[CommonKeys.attrs]
    ...
    def _get_dashboard_details(self, entity: AtlasEntityWithExtInfo) -> Dict:
        try:
            attributes = entity.entity[CommonKeys.attrs]
            relationships = entity.entity[CommonKeys.rels]
            ...
            for k, v in referred_entities.items():
                ...
                _attributes = v[CommonKeys.attrs]
    ...
    def _get_dashboard(self, qualified_name: str) -> AtlasEntityWithExtInfo:
        ...
        entity = self.client.entity.get_entity_by_attribute(type_name=DashboardTypes.metadata,
                                                            uniq_attributes=[(CommonKeys.qn, qualified_name)])
    ...
    def get_dashboard_description(self, *, id: str) -> Description:
        ...
        """
        entity = self.client.entity.get_entity_by_attribute(type_name=DashboardTypes.metadata,
                                                            uniq_attributes=[(CommonKeys.qn, id)])
        ...
        for dashboard in entities.entities:
            try:
                if dashboard.status == Status.ACTIVE:
                    executions = [entities['referredEntities'].get(e.get('guid'))[CommonKeys.attrs]
                                  for e in
                                  self._filter_active(dashboard[CommonKeys.rels].get('executions', []))]
    ...
    def get_resources_using_table(self, *,
        ...
        guids = [d.get('guid') for d in self._filter_active(table.entity[CommonKeys.rels].get(resource, []))]
